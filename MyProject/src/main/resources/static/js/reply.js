...
async function getReply(bno) {
    ...
    const response = await axios.get(`${contextPath}replies/list/${bno}`);
...
async function getList(bno, page, size, goLast) {
    ...
    const result = await axios.get(`${contextPath}replies/list/${bno}`, { params: {page, size} });
...
async function getReply(rno) {
    const response = await axios.get(`${contextPath}replies/${rno}`);
    return response.data;
}

// 4. 게시글에 대한 댓글 수정
async function modifyReply(replyObj) {
    const response = await axios.put(`${contextPath}replies/${replyObj.rno}`,replyObj);
    return response.data;
}

// 5. 게시글에 대한 댓글 삭제
async function removeReply(rno) {
    const response = await axios.delete(`${contextPath}replies/${rno}`);
    return response.data;
}
